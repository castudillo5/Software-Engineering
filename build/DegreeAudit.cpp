...
std::string DegreeAudit::toJson() const {
	...
	for (const CategoryProgress& cp : categoryProgress) {
		json += ",{";
		json += "\"name\":\"" + cp.categoryName + "\",";
		json += "\"total\":"     + std::to_string(cp.requiredHours)  + ",";
		json += "\"completed\":" + std::to_string(cp.completedHours) + ",";
		json += "\"courses\":[";
		// Missing mandatory courses as remaining items
		for (size_t i = 0; i < cp.missingMandatory.size(); i++) {
			json += "{\"code\":\"" + cp.missingMandatory[i] + "\",\"name\":\"\","
			        "\"completed\":false,\"inProgress\":false}";
			if (i < cp.missingMandatory.size() - 1) json += ",";
		}
		json += "]}";
	}
	...
	for (const FlexProgress& fp : flexProgress) {
		json += ",{";
		json += "\"name\":\"" + fp.categoryName + " (" + fp.categoryCode + ")\",";
		json += "\"total\":"     + std::to_string(fp.requiredHours)  + ",";
		json += "\"completed\":" + std::to_string(fp.completedHours) + ",";
		json += "\"courses\":[";
		if (!fp.satisfiedByCourse.empty()) {
			json += "{\"code\":\"" + fp.satisfiedByCourse + "\",\"name\":\"\","
			        "\"completed\":true,\"inProgress\":false}";
		}
		json += "]}";
	}
