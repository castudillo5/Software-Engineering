...
private:
	...
	// Cached pointers to the student and plan from the most recent run().
	// toJson() uses these to enrich the categories array with each
	// requirement's approved course list and per-course completion status.
	// They live as long as the original objects do — never owned.
	const Student* lastStudent = nullptr;
	const DegreePlan* lastPlan = nullptr;
