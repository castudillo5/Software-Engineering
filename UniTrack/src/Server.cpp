...
void saveStudentToFile(const Student& student) {
	std::ofstream outfile("data/student_data.txt");
...
bool loadStudentFromFile(Student& student) {
	std::ifstream infile("data/student_data.txt");
	if (!infile.is_open()) {
		std::cout << "[load] data/student_data.txt not found" << std::endl;
		return false;
	}

	std::cout << "[load] reading data/student_data.txt..." << std::endl;
...
static void loadDismissedAlerts() {
	std::ifstream infile("data/dismissed_alerts.txt");
...
static void saveDismissedAlerts() {
	std::ofstream outfile("data/dismissed_alerts.txt");
	if (!outfile.is_open()) return;
...
int main() {
	...
	std::vector<Course> catalog = Course::loadFromFile("data/Schedule_with_Credits.csv");
	g_catalog = &catalog;
	...
	// ── ROUTE: PUT /record ─────────────────────────────────────────────
	// Updates a record's grade. An empty newGrade is allowed and means
	// "clear the grade" — used by the schedule page's "What if?"
	// dropdown when the user picks the blank reset option to remove
	// their projection.
